...
contract Crowdfunding {
    ...
    function refund() public payable {
        require(block.timestamp > timeDeadline,"Deadline has not been Passed");
        require(funders[msg.sender] > 0, "You are not a Funder");
        payable(msg.sender).transfer(funders[msg.sender]);
    }

    function getTimeDeadline() public view returns (uint256) {
        return timeDeadline;
    }


    function getFavourcount(uint256 index) public view returns (uint256){
        Petitions storage _petition = petition[index];
        return _petition.favourCount;
    }

    function getnoOfFunders() public view returns (uint256){
        return noOfFunders.length;
    }
}
